...
struct WinSetupApiDevice {
   ...
   *
   * > **Do not attempt to parse the device path symbolic name.** > > The device
   * path can be reused across system starts.
   *
   ...
   * attempt to parse a symbolic link name.** > > Symbolic links for device
   * interface instances can be used across system > boots.
   *
   ...
   * instance of the specified device interface class. > > **The caller must
   * treat `SymbolicLinkName` as opaque** and **must not** > disassemble it.
   *
   * Examples:
   *
   * -
   ...
   * "PCI\\VEN_10DE&DEV_2584&CC_030000", "PCI\\VEN_10DE&DEV_2584&CC_0300" ] ```
   *
...
struct WinEdidInfo {
  ...
  /**
   * Normalized join key, with trailing `_0` removed from  {@link
   * wmi_instance_name } .
   *
   * E.g.:
   *
 ...
 */
enum class WinDisplayConnectorType : int {
  COMPONENT_VIDEO,
  COMPOSITE_VIDEO,
...
enum class WinScanLineOrder : int {
...
enum class WinStableIdSource : int {
  EDID_HASH_KEY,
  EDID_PARSED_KEY,
  LOCATION_PORT_KEY,
  MONITOR_PATH_KEY,
...
struct WinStableIdEntry {
  std::string key;
  WinStableIdSource source;
};

struct WinStableIds {
  std::vector<WinStableIdEntry> port_independent;
  std::vector<WinStableIdEntry> port_specific;
};

enum class WinColorEncoding : int {
  RGB,
  UNSPECIFIED,
  YCBCR420,
  YCBCR422,
...
enum class WinDxgiColorSpace : int {
  CUSTOM,
  RESERVED,
...
struct WinDisplay {
  /**
   * Effectively a reformatted version of  {@link  adapter_instance_id } , plus
   * a static, hard-coded device interface class GUID for adapters
   ...
   * system's device tree. > > The creation of the device instance ID for a
   * device uses the bus driver > reported device ID value, instance ID value,
   * and the UniqueID member of > the DEVICE_CAPABILITIES structure as input in
   * order to create the unique > device instance ID for this device on the
   * system. > > ``` > PCI\VEN_1000&DEV_0001&SUBSYS_00000000&REV_02\1&08 > ```
   */
   ...
   * Loading](https://reactos.org/wiki/Techwiki:Win32k/display_driver_loading),
   ...
   */
  std::optional<std::string> edid_data_string;
   ...
   */
  std::optional<std::string> edid_parsed_key;
  /**
   ...
   *
   * 1. EDID "monitor descriptor" name (e.g., `"DELL ST2320L"`) 2. `"Remote
   * Desktop"` or
   * `"Remote Desktop #N"` if RDP 3. `"Virtual Machine"` or `"Virtual Machine
   * #N"` if a VM 4. 7-digit Windows EDID identifier (e.g., `"SAM73A5"` or
   * `"DELF023"`) 5. Short-lived Windows display number (e.g., `"DISPLAY1"`)
   */
  std::optional<std::string> friendly_name;
  /**
   * This value MIGHT be `false` under the following conditions:
   *
   * - Unused connectors on the GPU:   - Many drivers expose one IDXGIOutput per
   * physical connector     (HDMI/DP/DVI), even if nothing is plugged in.   -
   * Those "ports" can enumerate, but they are not part of the desktop, so
   * AttachedToDesktop is false.
   *
   * - A monitor is connected but disabled in Display Settings:   - Example: you
   * have 2 monitors connected, but Windows is set to     "Show only on 1" (or
   * you've "Disconnect this display" for the other).     That other output can
   * still exist, but it is not attached, so false.
   *
   * TODO(acdvorak): Clarify the difference between  {@link
   * has_interactive_desktop  }  and
   ...
   * (the
   * `{4d36e96e-e325-11ce-bfc1-08002be10318}` part, which dictates that the
   * device is a "Monitor") and assigns it the next available 4-digit number
   * starting from `0000`.
   *
   ...
   *
   * - **DriverDesc**: The friendly, human-readable name of the monitor   (e.g.,
   * "Generic PnP Monitor", "Generic Non-PnP Monitor", or   "Samsung
   * SyncMaster").
   *
...
void from_json(const json& j, WinStableIdEntry& x);
void to_json(json& j, const WinStableIdEntry& x);

void from_json(const json& j, WinStableIds& x);
void to_json(json& j, const WinStableIds& x);
...
void from_json(const json& j, WinStableIdSource& x);
void to_json(json& j, const WinStableIdSource& x);
...
inline void from_json(const json& j, WinSetupApiDevice& x) {
  ...
  x.compatible_ids =
      get_stack_optional<std::vector<std::string>>(j, "compatible_ids");
  ...
  x.hardware_id =
      get_stack_optional<std::vector<std::string>>(j, "hardware_id");
  x.instance_id = get_stack_optional<std::string>(j, "instance_id");
  x.legacy_bus_type = get_stack_optional<uint32_t>(j, "legacy_bus_type");
  x.location_information =
      get_stack_optional<std::string>(j, "location_information");
  x.location_paths =
      get_stack_optional<std::vector<std::string>>(j, "location_paths");
  x.mfg = get_stack_optional<std::string>(j, "mfg");
  x.physical_device_object_name =
      get_stack_optional<std::string>(j, "physical_device_object_name");
  x.service = get_stack_optional<std::string>(j, "service");
  x.ui_number = get_stack_optional<uint32_t>(j, "ui_number");
  x.ui_number_desc_format =
      get_stack_optional<std::string>(j, "ui_number_desc_format");
}
...
inline void from_json(const json& j, WinAdvancedColorInfo& x) {
  x.active_color_mode =
      get_stack_optional<WinActiveColorMode>(j, "active_color_mode");
  ...
  x.is_advanced_color_force_disabled =
      j.at("is_advanced_color_force_disabled").get<bool>();
  x.is_advanced_color_limited_by_policy =
      j.at("is_advanced_color_limited_by_policy").get<bool>();
  x.is_advanced_color_supported =
      j.at("is_advanced_color_supported").get<bool>();
  x.is_high_dynamic_range_supported =
      j.at("is_high_dynamic_range_supported").get<bool>();
  x.is_high_dynamic_range_user_enabled =
      j.at("is_high_dynamic_range_user_enabled").get<bool>();
...
inline void to_json(json& j, const WinAdvancedColorInfo& x) {
  ...
  j["is_advanced_color_limited_by_policy"] =
      x.is_advanced_color_limited_by_policy;
  ...
  j["is_high_dynamic_range_user_enabled"] =
      x.is_high_dynamic_range_user_enabled;
...
inline void from_json(const json& j, WinEdidInfo& x) {
  ...
  x.max_horizontal_image_size_mm =
      get_stack_optional<double>(j, "max_horizontal_image_size_mm");
  x.max_vertical_image_size_mm =
      get_stack_optional<double>(j, "max_vertical_image_size_mm");
  ...
  x.user_friendly_name =
      get_stack_optional<std::string>(j, "user_friendly_name");
  x.video_output_technology_type = get_stack_optional<WmiVideoOutputTechnology>(
      j, "video_output_technology_type");
  ...
  x.year_of_manufacture =
      get_stack_optional<uint16_t>(j, "year_of_manufacture");
}
...
inline void from_json(const json& j, WinStableIdEntry& x) {
  x.key = j.at("key").get<std::string>();
  x.source = j.at("source").get<WinStableIdSource>();
}

inline void to_json(json& j, const WinStableIdEntry& x) {
  j = json::object();
  j["key"] = x.key;
  j["source"] = x.source;
}

inline void from_json(const json& j, WinStableIds& x) {
  x.port_independent =
      j.at("port_independent").get<std::vector<WinStableIdEntry>>();
  x.port_specific = j.at("port_specific").get<std::vector<WinStableIdEntry>>();
}

inline void to_json(json& j, const WinStableIds& x) {
  j = json::object();
  j["port_independent"] = x.port_independent;
  j["port_specific"] = x.port_specific;
}

inline void from_json(const json& j, WinStandardColorInfo& x) {
  x.bits_per_channel =
      get_stack_optional<WinBitsPerColorChannel>(j, "bits_per_channel");
  x.color_encoding = get_stack_optional<WinColorEncoding>(j, "color_encoding");
  x.dxgi_color_space =
      get_stack_optional<WinDxgiColorSpace>(j, "dxgi_color_space");
  x.is_hdr_enabled = j.at("is_hdr_enabled").get<bool>();
  x.is_hdr_supported = j.at("is_hdr_supported").get<bool>();
  x.max_full_frame_luminance_nits =
      get_stack_optional<double>(j, "max_full_frame_luminance_nits");
...
inline void from_json(const json& j, WinDisplay& x) {
  x.adapter_device_path =
      get_stack_optional<std::string>(j, "adapter_device_path");
  x.adapter_friendly_name =
      get_stack_optional<std::string>(j, "adapter_friendly_name");
  x.adapter_hardware_id =
      get_stack_optional<std::string>(j, "adapter_hardware_id");
  x.adapter_instance_id =
      get_stack_optional<std::string>(j, "adapter_instance_id");
  x.adapter_registry_key =
      get_stack_optional<std::string>(j, "adapter_registry_key");
  x.advanced_color_info =
      get_stack_optional<WinAdvancedColorInfo>(j, "advanced_color_info");
  x.bounds = j.at("bounds").get<WinScreenRectangle>();
  x.dpi_scaling_percent =
      get_stack_optional<uint32_t>(j, "dpi_scaling_percent");
  x.edid_data_string = get_stack_optional<std::string>(j, "edid_data_string");
  x.edid_info = get_stack_optional<WinEdidInfo>(j, "edid_info");
  x.edid_parsed_key = get_stack_optional<std::string>(j, "edid_parsed_key");
  x.friendly_name = get_stack_optional<std::string>(j, "friendly_name");
  x.is_attached_to_desktop =
      get_stack_optional<bool>(j, "is_attached_to_desktop");
  x.is_primary = j.at("is_primary").get<bool>();
  x.monitor_device_path =
      get_stack_optional<std::string>(j, "monitor_device_path");
  x.monitor_driver_key =
      get_stack_optional<std::string>(j, "monitor_driver_key");
  x.monitor_instance_id =
      get_stack_optional<std::string>(j, "monitor_instance_id");
  x.monitor_path_key = get_stack_optional<std::string>(j, "monitor_path_key");
  x.monitor_registry_key =
      get_stack_optional<std::string>(j, "monitor_registry_key");
  x.monitor_serial_string =
      get_stack_optional<std::string>(j, "monitor_serial_string");
  x.monitor_string = get_stack_optional<std::string>(j, "monitor_string");
  x.physical_connector_type =
      get_stack_optional<WinDisplayConnectorType>(j, "physical_connector_type");
  x.primary_port_key = get_stack_optional<std::string>(j, "primary_port_key");
  x.refresh_rate_denominator =
      get_stack_optional<uint32_t>(j, "refresh_rate_denominator");
  x.refresh_rate_hz = get_stack_optional<double>(j, "refresh_rate_hz");
  x.refresh_rate_numerator =
      get_stack_optional<uint32_t>(j, "refresh_rate_numerator");
  x.rotation_deg =
      get_stack_optional<WinDisplayRotationDegrees>(j, "rotation_deg");
  x.scan_line_ordering =
      get_stack_optional<WinScanLineOrder>(j, "scan_line_ordering");
  x.setup_api_devices =
      j.at("setup_api_devices").get<std::vector<WinSetupApiDeviceCatalog>>();
  x.short_lived_identifier = j.at("short_lived_identifier").get<std::string>();
  x.stable_id = get_stack_optional<std::string>(j, "stable_id");
  x.stable_id_candidates =
      get_stack_optional<std::vector<std::string>>(j, "stable_id_candidates");
  x.stable_id_source =
      get_stack_optional<WinStableIdSource>(j, "stable_id_source");
  x.stable_ids = get_stack_optional<WinStableIds>(j, "stable_ids");
  x.standard_color_info =
      j.at("standard_color_info").get<WinStandardColorInfo>();
...
inline void to_json(json& j, const WinDisplay& x) {
  ...
  if (x.edid_data_string) {
    j["edid_data_string"] = x.edid_data_string;
  }
  if (x.edid_info) {
    j["edid_info"] = x.edid_info;
  }
  if (x.edid_parsed_key) {
    j["edid_parsed_key"] = x.edid_parsed_key;
  }
  if (x.friendly_name) {
    j["friendly_name"] = x.friendly_name;
  }
...
inline void from_json(const json& j, WinDisplayProberJson& x) {
  x.all_setup_api_devices =
      j.at("all_setup_api_devices").get<WinSetupApiDeviceCatalog>();
...
inline void from_json(const json& j, WinDisplayRotationDegrees& x) {
  ...
  switch (value) {
    case 0:
      x = WinDisplayRotationDegrees::VALUE_0;
      break;
    case 90:
      x = WinDisplayRotationDegrees::VALUE_90;
      break;
    case 180:
      x = WinDisplayRotationDegrees::VALUE_180;
      break;
    case 270:
      x = WinDisplayRotationDegrees::VALUE_270;
      break;
    default:
...
inline void to_json(json& j, const WinDisplayRotationDegrees& x) {
  switch (x) {
    case WinDisplayRotationDegrees::VALUE_0:
      j = 0;
      break;
    case WinDisplayRotationDegrees::VALUE_90:
      j = 90;
      break;
    case WinDisplayRotationDegrees::VALUE_180:
      j = 180;
      break;
    case WinDisplayRotationDegrees::VALUE_270:
      j = 270;
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinDisplayRotationDegrees\": " +
          std::to_string(static_cast<int64_t>(x)));
  }
}
...
inline void from_json(const json& j, WinBitsPerColorChannel& x) {
  ...
  switch (value) {
    case 0:
      x = WinBitsPerColorChannel::VALUE_0;
      break;
    case 6:
      x = WinBitsPerColorChannel::VALUE_6;
      break;
    case 8:
      x = WinBitsPerColorChannel::VALUE_8;
      break;
    case 10:
      x = WinBitsPerColorChannel::VALUE_10;
      break;
    case 12:
      x = WinBitsPerColorChannel::VALUE_12;
      break;
    case 14:
      x = WinBitsPerColorChannel::VALUE_14;
      break;
    case 16:
      x = WinBitsPerColorChannel::VALUE_16;
      break;
    default:
      throw std::runtime_error("Input JSON does not conform to schema!");
  }
...
inline void to_json(json& j, const WinBitsPerColorChannel& x) {
  switch (x) {
    case WinBitsPerColorChannel::VALUE_0:
      j = 0;
      break;
    case WinBitsPerColorChannel::VALUE_6:
      j = 6;
      break;
    case WinBitsPerColorChannel::VALUE_8:
      j = 8;
      break;
    case WinBitsPerColorChannel::VALUE_10:
      j = 10;
      break;
    case WinBitsPerColorChannel::VALUE_12:
      j = 12;
      break;
    case WinBitsPerColorChannel::VALUE_14:
      j = 14;
      break;
    case WinBitsPerColorChannel::VALUE_16:
      j = 16;
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinBitsPerColorChannel\": " +
          std::to_string(static_cast<int64_t>(x)));
  }
}
...
inline void from_json(const json& j, WmiVideoOutputTechnology& x) {
  ...
  switch (value) {
    case -2:
      x = WmiVideoOutputTechnology::UNINITIALIZED;
      break;
    case -1:
      x = WmiVideoOutputTechnology::OTHER;
      break;
    case 0:
      x = WmiVideoOutputTechnology::VGA;
      break;
    case 1:
      x = WmiVideoOutputTechnology::SVIDEO;
      break;
    case 2:
      x = WmiVideoOutputTechnology::COMPOSITE_VIDEO;
      break;
    case 3:
      x = WmiVideoOutputTechnology::COMPONENT_VIDEO;
      break;
    case 4:
      x = WmiVideoOutputTechnology::DVI;
      break;
    case 5:
      x = WmiVideoOutputTechnology::HDMI;
      break;
    case 6:
      x = WmiVideoOutputTechnology::LVDS;
      break;
    case 8:
      x = WmiVideoOutputTechnology::D_JPN;
      break;
    case 9:
      x = WmiVideoOutputTechnology::SDI;
      break;
    case 10:
      x = WmiVideoOutputTechnology::DISPLAYPORT_EXTERNAL;
      break;
    case 11:
      x = WmiVideoOutputTechnology::DISPLAYPORT_EMBEDDED;
      break;
    case 12:
      x = WmiVideoOutputTechnology::UDI_EXTERNAL;
      break;
    case 13:
      x = WmiVideoOutputTechnology::UDI_EMBEDDED;
      break;
    case 14:
      x = WmiVideoOutputTechnology::SDTVDONGLE;
      break;
    case 15:
      x = WmiVideoOutputTechnology::MIRACAST;
      break;
    case 16:
      x = WmiVideoOutputTechnology::INDIRECT_WIRED;
      break;
    case 2147483648:
      x = WmiVideoOutputTechnology::INTERNAL;
      break;
    default:
      throw std::runtime_error("Input JSON does not conform to schema!");
  }
...
inline void to_json(json& j, const WmiVideoOutputTechnology& x) {
  switch (x) {
    case WmiVideoOutputTechnology::UNINITIALIZED:
      j = -2;
      break;
    case WmiVideoOutputTechnology::OTHER:
      j = -1;
      break;
    case WmiVideoOutputTechnology::VGA:
      j = 0;
      break;
    case WmiVideoOutputTechnology::SVIDEO:
      j = 1;
      break;
    case WmiVideoOutputTechnology::COMPOSITE_VIDEO:
      j = 2;
      break;
    case WmiVideoOutputTechnology::COMPONENT_VIDEO:
      j = 3;
      break;
    case WmiVideoOutputTechnology::DVI:
      j = 4;
      break;
    case WmiVideoOutputTechnology::HDMI:
      j = 5;
      break;
    case WmiVideoOutputTechnology::LVDS:
      j = 6;
      break;
    case WmiVideoOutputTechnology::D_JPN:
      j = 8;
      break;
    case WmiVideoOutputTechnology::SDI:
      j = 9;
      break;
    case WmiVideoOutputTechnology::DISPLAYPORT_EXTERNAL:
      j = 10;
      break;
    case WmiVideoOutputTechnology::DISPLAYPORT_EMBEDDED:
      j = 11;
      break;
    case WmiVideoOutputTechnology::UDI_EXTERNAL:
      j = 12;
      break;
    case WmiVideoOutputTechnology::UDI_EMBEDDED:
      j = 13;
      break;
    case WmiVideoOutputTechnology::SDTVDONGLE:
      j = 14;
      break;
    case WmiVideoOutputTechnology::MIRACAST:
      j = 15;
      break;
    case WmiVideoOutputTechnology::INDIRECT_WIRED:
      j = 16;
      break;
    case WmiVideoOutputTechnology::INTERNAL:
      j = 2147483648;
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WmiVideoOutputTechnology\": " +
          std::to_string(static_cast<int64_t>(x)));
  }
}
inline void from_json(const json& j, WinActiveColorMode& x) {
  if (j == "hdr")
    x = WinActiveColorMode::HDR;
  else if (j == "sdr")
    x = WinActiveColorMode::SDR;
  else if (j == "unspecified")
    x = WinActiveColorMode::UNSPECIFIED;
  else if (j == "wcg")
    x = WinActiveColorMode::WCG;
  else {
    throw std::runtime_error("Input JSON does not conform to schema!");
  }
}
...
inline void to_json(json& j, const WinActiveColorMode& x) {
  switch (x) {
    case WinActiveColorMode::HDR:
      j = "hdr";
      break;
    case WinActiveColorMode::SDR:
      j = "sdr";
      break;
    case WinActiveColorMode::UNSPECIFIED:
      j = "unspecified";
      break;
    case WinActiveColorMode::WCG:
      j = "wcg";
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinActiveColorMode\": " +
...
inline void from_json(const json& j, WinDisplayConnectorType& x) {
  static std::unordered_map<std::string, WinDisplayConnectorType> enumValues{
      ...
      {"displayport_usb_tunnel",
       WinDisplayConnectorType::DISPLAYPORT_USB_TUNNEL},
...
inline void to_json(json& j, const WinDisplayConnectorType& x) {
  switch (x) {
    case WinDisplayConnectorType::COMPONENT_VIDEO:
      j = "component_video";
      break;
    case WinDisplayConnectorType::COMPOSITE_VIDEO:
      j = "composite_video";
      break;
    case WinDisplayConnectorType::DISPLAYPORT_EMBEDDED:
      j = "displayport_embedded";
      break;
    case WinDisplayConnectorType::DISPLAYPORT_EXTERNAL:
      j = "displayport_external";
      break;
    case WinDisplayConnectorType::DISPLAYPORT_USB_TUNNEL:
      j = "displayport_usb_tunnel";
      break;
    case WinDisplayConnectorType::DVI:
      j = "dvi";
      break;
    case WinDisplayConnectorType::D_JPN:
      j = "d_jpn";
      break;
    case WinDisplayConnectorType::FAKE:
      j = "fake";
      break;
    case WinDisplayConnectorType::HDMI:
      j = "hdmi";
      break;
    case WinDisplayConnectorType::INDIRECT_VIRTUAL:
      j = "indirect_virtual";
      break;
    case WinDisplayConnectorType::INDIRECT_WIRED:
      j = "indirect_wired";
      break;
    case WinDisplayConnectorType::INTERNAL:
      j = "internal";
      break;
    case WinDisplayConnectorType::LVDS:
      j = "lvds";
      break;
    case WinDisplayConnectorType::MIRACAST:
      j = "miracast";
      break;
    case WinDisplayConnectorType::OTHER:
      j = "other";
      break;
    case WinDisplayConnectorType::RDP:
      j = "rdp";
      break;
    case WinDisplayConnectorType::SDI:
      j = "sdi";
      break;
    case WinDisplayConnectorType::SDTVDONGLE:
      j = "sdtvdongle";
      break;
    case WinDisplayConnectorType::SVIDEO:
      j = "svideo";
      break;
    case WinDisplayConnectorType::UDI_EMBEDDED:
      j = "udi_embedded";
      break;
    case WinDisplayConnectorType::UDI_EXTERNAL:
      j = "udi_external";
      break;
    case WinDisplayConnectorType::VGA:
      j = "vga";
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinDisplayConnectorType\": " +
          std::to_string(static_cast<int>(x)));
  }
}

inline void from_json(const json& j, WinScanLineOrder& x) {
  if (j == "interlaced_lower_field_first")
    x = WinScanLineOrder::INTERLACED_LOWER_FIELD_FIRST;
  else if (j == "interlaced_upper_field_first")
    x = WinScanLineOrder::INTERLACED_UPPER_FIELD_FIRST;
  else if (j == "progressive")
    x = WinScanLineOrder::PROGRESSIVE;
  else if (j == "unspecified")
    x = WinScanLineOrder::UNSPECIFIED;
  else {
    throw std::runtime_error("Input JSON does not conform to schema!");
  }
}
...
inline void to_json(json& j, const WinScanLineOrder& x) {
  switch (x) {
    case WinScanLineOrder::INTERLACED_LOWER_FIELD_FIRST:
      j = "interlaced_lower_field_first";
      break;
    case WinScanLineOrder::INTERLACED_UPPER_FIELD_FIRST:
      j = "interlaced_upper_field_first";
      break;
    case WinScanLineOrder::PROGRESSIVE:
      j = "progressive";
      break;
    case WinScanLineOrder::UNSPECIFIED:
      j = "unspecified";
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinScanLineOrder\": " +
          std::to_string(static_cast<int>(x)));
  }
}

inline void from_json(const json& j, WinStableIdSource& x) {
  if (j == "edid_hash_key")
    x = WinStableIdSource::EDID_HASH_KEY;
  else if (j == "edid_parsed_key")
    x = WinStableIdSource::EDID_PARSED_KEY;
  else if (j == "location_port_key")
    x = WinStableIdSource::LOCATION_PORT_KEY;
  else if (j == "monitor_path_key")
    x = WinStableIdSource::MONITOR_PATH_KEY;
  else if (j == "primary_port_key")
    x = WinStableIdSource::PRIMARY_PORT_KEY;
  else {
    throw std::runtime_error("Input JSON does not conform to schema!");
  }
}

inline void to_json(json& j, const WinStableIdSource& x) {
  switch (x) {
    case WinStableIdSource::EDID_HASH_KEY:
      j = "edid_hash_key";
      break;
    case WinStableIdSource::EDID_PARSED_KEY:
      j = "edid_parsed_key";
      break;
    case WinStableIdSource::LOCATION_PORT_KEY:
      j = "location_port_key";
      break;
    case WinStableIdSource::MONITOR_PATH_KEY:
      j = "monitor_path_key";
      break;
    case WinStableIdSource::PRIMARY_PORT_KEY:
      j = "primary_port_key";
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinStableIdSource\": " +
          std::to_string(static_cast<int>(x)));
  }
}
...
inline void from_json(const json& j, WinColorEncoding& x) {
  if (j == "rgb")
    x = WinColorEncoding::RGB;
  else if (j == "unspecified")
    x = WinColorEncoding::UNSPECIFIED;
  else if (j == "ycbcr420")
    x = WinColorEncoding::YCBCR420;
  else if (j == "ycbcr422")
    x = WinColorEncoding::YCBCR422;
  else if (j == "ycbcr444")
    x = WinColorEncoding::YCBCR444;
  else {
    throw std::runtime_error("Input JSON does not conform to schema!");
  }
}
...
inline void to_json(json& j, const WinColorEncoding& x) {
  switch (x) {
    case WinColorEncoding::RGB:
      j = "rgb";
      break;
    case WinColorEncoding::UNSPECIFIED:
      j = "unspecified";
      break;
    case WinColorEncoding::YCBCR420:
      j = "ycbcr420";
      break;
    case WinColorEncoding::YCBCR422:
      j = "ycbcr422";
      break;
    case WinColorEncoding::YCBCR444:
      j = "ycbcr444";
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinColorEncoding\": " +
          std::to_string(static_cast<int>(x)));
  }
}
...
inline void from_json(const json& j, WinDxgiColorSpace& x) {
  static std::unordered_map<std::string, WinDxgiColorSpace> enumValues{
      ...
      {"rgb_full_g2084_none_p2020",
       WinDxgiColorSpace::RGB_FULL_G2084_NONE_P2020},
      ...
      {"rgb_studio_g2084_none_p2020",
       WinDxgiColorSpace::RGB_STUDIO_G2084_NONE_P2020},
      {"rgb_studio_g22_none_p2020",
       WinDxgiColorSpace::RGB_STUDIO_G22_NONE_P2020},
      {"rgb_studio_g22_none_p709", WinDxgiColorSpace::RGB_STUDIO_G22_NONE_P709},
      {"rgb_studio_g24_none_p2020",
       WinDxgiColorSpace::RGB_STUDIO_G24_NONE_P2020},
      {"rgb_studio_g24_none_p709", WinDxgiColorSpace::RGB_STUDIO_G24_NONE_P709},
      {"ycbcr_full_g22_left_p2020",
       WinDxgiColorSpace::YCBCR_FULL_G22_LEFT_P2020},
      ...
      {"ycbcr_full_g22_none_p709_x601",
       WinDxgiColorSpace::YCBCR_FULL_G22_NONE_P709_X601},
      {"ycbcr_full_ghlg_topleft_p2020",
       WinDxgiColorSpace::YCBCR_FULL_GHLG_TOPLEFT_P2020},
      {"ycbcr_studio_g2084_left_p2020",
       WinDxgiColorSpace::YCBCR_STUDIO_G2084_LEFT_P2020},
      {"ycbcr_studio_g2084_topleft_p2020",
       WinDxgiColorSpace::YCBCR_STUDIO_G2084_TOPLEFT_P2020},
      {"ycbcr_studio_g22_left_p2020",
       WinDxgiColorSpace::YCBCR_STUDIO_G22_LEFT_P2020},
      {"ycbcr_studio_g22_left_p601",
       WinDxgiColorSpace::YCBCR_STUDIO_G22_LEFT_P601},
      {"ycbcr_studio_g22_left_p709",
       WinDxgiColorSpace::YCBCR_STUDIO_G22_LEFT_P709},
      {"ycbcr_studio_g22_topleft_p2020",
       WinDxgiColorSpace::YCBCR_STUDIO_G22_TOPLEFT_P2020},
      {"ycbcr_studio_g24_left_p2020",
       WinDxgiColorSpace::YCBCR_STUDIO_G24_LEFT_P2020},
      {"ycbcr_studio_g24_left_p709",
       WinDxgiColorSpace::YCBCR_STUDIO_G24_LEFT_P709},
      {"ycbcr_studio_g24_topleft_p2020",
       WinDxgiColorSpace::YCBCR_STUDIO_G24_TOPLEFT_P2020},
      {"ycbcr_studio_ghlg_topleft_p2020",
       WinDxgiColorSpace::YCBCR_STUDIO_GHLG_TOPLEFT_P2020},
  };
  auto iter = enumValues.find(j.get<std::string>());
  if (iter != enumValues.end()) {
    x = iter->second;
...
inline void to_json(json& j, const WinDxgiColorSpace& x) {
  switch (x) {
    case WinDxgiColorSpace::CUSTOM:
      j = "custom";
      break;
    case WinDxgiColorSpace::RESERVED:
      j = "reserved";
      break;
    case WinDxgiColorSpace::RGB_FULL_G10_NONE_P709:
      j = "rgb_full_g10_none_p709";
      break;
    case WinDxgiColorSpace::RGB_FULL_G2084_NONE_P2020:
      j = "rgb_full_g2084_none_p2020";
      break;
    case WinDxgiColorSpace::RGB_FULL_G22_NONE_P2020:
      j = "rgb_full_g22_none_p2020";
      break;
    case WinDxgiColorSpace::RGB_FULL_G22_NONE_P709:
      j = "rgb_full_g22_none_p709";
      break;
    case WinDxgiColorSpace::RGB_STUDIO_G2084_NONE_P2020:
      j = "rgb_studio_g2084_none_p2020";
      break;
    case WinDxgiColorSpace::RGB_STUDIO_G22_NONE_P2020:
      j = "rgb_studio_g22_none_p2020";
      break;
    case WinDxgiColorSpace::RGB_STUDIO_G22_NONE_P709:
      j = "rgb_studio_g22_none_p709";
      break;
    case WinDxgiColorSpace::RGB_STUDIO_G24_NONE_P2020:
      j = "rgb_studio_g24_none_p2020";
      break;
    case WinDxgiColorSpace::RGB_STUDIO_G24_NONE_P709:
      j = "rgb_studio_g24_none_p709";
      break;
    case WinDxgiColorSpace::YCBCR_FULL_G22_LEFT_P2020:
      j = "ycbcr_full_g22_left_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_FULL_G22_LEFT_P601:
      j = "ycbcr_full_g22_left_p601";
      break;
    case WinDxgiColorSpace::YCBCR_FULL_G22_LEFT_P709:
      j = "ycbcr_full_g22_left_p709";
      break;
    case WinDxgiColorSpace::YCBCR_FULL_G22_NONE_P709_X601:
      j = "ycbcr_full_g22_none_p709_x601";
      break;
    case WinDxgiColorSpace::YCBCR_FULL_GHLG_TOPLEFT_P2020:
      j = "ycbcr_full_ghlg_topleft_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G2084_LEFT_P2020:
      j = "ycbcr_studio_g2084_left_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G2084_TOPLEFT_P2020:
      j = "ycbcr_studio_g2084_topleft_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G22_LEFT_P2020:
      j = "ycbcr_studio_g22_left_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G22_LEFT_P601:
      j = "ycbcr_studio_g22_left_p601";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G22_LEFT_P709:
      j = "ycbcr_studio_g22_left_p709";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G22_TOPLEFT_P2020:
      j = "ycbcr_studio_g22_topleft_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G24_LEFT_P2020:
      j = "ycbcr_studio_g24_left_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G24_LEFT_P709:
      j = "ycbcr_studio_g24_left_p709";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_G24_TOPLEFT_P2020:
      j = "ycbcr_studio_g24_topleft_p2020";
      break;
    case WinDxgiColorSpace::YCBCR_STUDIO_GHLG_TOPLEFT_P2020:
      j = "ycbcr_studio_ghlg_topleft_p2020";
      break;
    default:
      throw std::runtime_error(
          "Unexpected value in enumeration \"WinDxgiColorSpace\": " +
          std::to_string(static_cast<int>(x)));
  }
}
}  // namespace json
